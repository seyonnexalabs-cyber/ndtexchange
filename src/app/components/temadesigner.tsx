 ...
 * - Column labels (top, vertical leader line to topmost tube in col, text rotated)
 * - Hexagon shape fix (flat-to-flat apothem formula)
 * - Polygon shape with canvas drawing mode
 * - Column numbers NOT inside circles (external headers only)
 */
...
const MARGIN_LEFT = 38;   // px reserved for row labels
const MARGIN_TOP  = 38;   // px reserved for col labels
...
export default function TemaDesigner({ isTrial }: { isTrial?: boolean }) {
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Delete'||e.key==='Backspace'){
        if(selIds.size>0){
          setTubes(tubes.filter(t=>!selIds.has(t.id)));
  ...
  const css2w=useCallback((cx:number,cy:number)=>({
    x:(cx-panRef.current.x)/zoomRef.current,
    y:(cy-panRef.current.y)/zoomRef.current,
  }),[]);

  const w2css=useCallback((wx:number,wy:number)=>({
    x:panRef.current.x+wx*zoomRef.current,
    y:panRef.current.y+wy*zoomRef.current,
  }),[]);
  ...
  const draw=useCallback(()=>{
    ...
    // Margin areas (white + light border)
    ctx.fillStyle="#ffffff";
    ctx.fillRect(0,0,CW,MARGIN_TOP);
    ctx.fillRect(0,0,MARGIN_LEFT,CH);
    ...
    ctx.fillRect(MARGIN_LEFT,MARGIN_TOP,CW-MARGIN_LEFT,CH-MARGIN_TOP);

    // Grid
    if(showGrid&&z>0.3){
      const gStep=50*z;
      const ox=((p.x%gStep)+gStep)%gStep, oy=((p.y%gStep)+gStep)%gStep;
      ctx.strokeStyle="rgba(148,163,184,0.2)"; ctx.lineWidth=0.5;
      for(let x=MARGIN_LEFT+(ox%gStep);x<CW;x+=gStep){
        ctx.beginPath(); ctx.moveTo(x,MARGIN_TOP); ctx.lineTo(x,CH); ctx.stroke();
      }
      for(let y=MARGIN_TOP+(oy%gStep);y<CH;y+=gStep){
        ctx.beginPath(); ctx.moveTo(MARGIN_LEFT,y); ctx.lineTo(CW,y); ctx.stroke();
      }
    }
    ...
    for(const t of tubes){
      ...
      let col:string;
      if(colorMode==="pass") col=passColor(t.pass);
      else if(colorMode==="row") col=rowColor(t.row);
      else col="#334155";
      ...
      if(tr>6){ctx.beginPath();ctx.arc(tx,ty,tr*0.5,0,Math.PI*2);ctx.strokeStyle=col+"30";ctx.lineWidth=0.5;ctx.stroke();}
    }

    // ── Row labels with horizontal leader lines ──────────────────────────
    if(showLabels&&layout){
      ...
      for(const r of uRows){
        ...
        const ty=p.y+leftmost.y*z;
        if(ty<MARGIN_TOP+4||ty>CH-4) continue;
        const tx=p.x+leftmost.x*z, tr=Math.max(leftmost.r*z,1.2);
        const col=rowColor(r);
        // Leader line from left margin to tube edge
        ctx.strokeStyle=col+"70"; ctx.lineWidth=0.7; ctx.setLineDash([3,3]);
        ctx.beginPath();
        ctx.moveTo(MARGIN_LEFT-2,ty); ctx.lineTo(tx-tr-3,ty); ctx.stroke();
        ctx.setLineDash([]);
        // Row label in left margin
        ctx.fillStyle=col;
        ctx.textAlign="right"; ctx.textBaseline="middle";
        ctx.fillText(`R${r+1}`,MARGIN_LEFT-5,ty);
      }
      ctx.restore();
    }

    // ── Column labels with vertical leader lines ─────────────────────────
    if(showLabels&&layout){
      const uCols=Array.from(new Set(tubes.map(t=>t.col))).sort((a,b)=>a-b);
      ...
      for(const c of uCols){
        ...
        const tx=p.x+topmost.x*z;
        if(tx<MARGIN_LEFT+4||tx>CW-4) continue;
        if(tx-lastLabelX<18) continue; // skip crowded labels
        lastLabelX=tx;
        const ty=p.y+topmost.y*z, tr=Math.max(topmost.r*z,1.2);
        // Vertical leader line from top margin border down to tube top
        ctx.strokeStyle="rgba(100,116,139,0.35)"; ctx.lineWidth=0.6; ctx.setLineDash([2,3]);
        ctx.beginPath(); ctx.moveTo(tx,MARGIN_TOP); ctx.lineTo(tx,ty-tr-3); ctx.stroke();
        ctx.setLineDash([]);
        // Column label — rotated -90° so text reads bottom-up
        ctx.save();
        ctx.translate(tx, MARGIN_TOP/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillStyle=C.text3;
        ctx.textAlign="center"; ctx.textBaseline="middle";
        ctx.fillText(`C${c+1}`,0,0);
        ctx.restore();
      }
      ctx.restore();
    }
    ...
    if(polyMode&&polyWip.length>0){
      ...
      ctx.fillText(txt,CW/2,CH-18); ctx.textBaseline="alphabetic";
      ctx.restore();
    }
  ...
  const onMouseDown=useCallback((e:React.MouseEvent<HTMLCanvasElement>)=>{
    ...
    if(e.button===0){
      ...
      if(hit){
        ...
        const w=css2w(css.x,css.y);
        selStart.current={wx:w.x,wy:w.y};
        setSelBox({x1:css.x,y1:css.y,x2:css.x,y2:css.y});
      }
    }
  },[polyMode,tool,pan,evToCSS,css2w,hitTest]);

  const onMouseMove=useCallback((e:React.MouseEvent<HTMLCanvasElement>)=>{
    const css=evToCSS(e);
    if(polyMode){ polyMousePos.current=css; draw(); return; }
    if(panDrag.current){
      const nx=panDrag.current.px+(e.clientX-panDrag.current.mx);
      const ny=panDrag.current.py+(e.clientY-panDrag.current.my);
      panRef.current={x:nx,y:ny}; setPan({x:nx,y:ny}); return;
    }
    if(selStart.current){
      const s=selStart.current, sc=w2css(s.wx,s.wy);
      setSelBox({x1:sc.x,y1:sc.y,x2:css.x,y2:css.y}); return;
    }
  ...
  const onWheel=useCallback((e:React.WheelEvent<HTMLCanvasElement>)=>{
    ...
    setZoom(prev=>{
      ...
      const nx=css.x+(panRef.current.x-css.x)*nz/prev;
      const ny=css.y+(panRef.current.y-css.y)*nz/prev;
  ...
  const fitView=useCallback(()=>{
    ...
    zoomRef.current=z;
    panRef.current={x:MARGIN_LEFT+cw/2-cx*z,y:MARGIN_TOP+ch/2-cy*z};
  ...
  const insertRowAfter=useCallback((row:number)=>{
    ...
    const pitchY=layout.pattern==='triangular'||layout.pattern==='rotated-triangular'
      ?layout.pitchMm*Math.sqrt(3)/2 : layout.pitchMm;
  ...
  const recalc=()=>{
    if(!layout) return;
    setTubes(recalcRowsCols(tubes,layout.pitchMm));
    setNeedRecalc(false);
  };
  ...
  return(
    <div style={{display:"flex",flex:1,height:"100%",overflow:"hidden",fontFamily:F,background:C.bg}}>
      ...
      <div style={{width:260,flexShrink:0,background:C.panel,borderRight:`1px solid ${C.border}`,
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px 16px"}}>
          ...
          {cfg.shape.type==="polygon"&&(
            <div style={{marginBottom:6}}>
              ...
              <div style={{display:"flex",gap:4}}>
                <Btn size="xs" variant={polyMode?"primary":"default"} onClick={()=>{setPolyMode(true);setPolyWip([]);}}>
                  {polyMode?"Drawing…":"✏ Draw"}
                </Btn>
                {polyMode&&<Btn size="xs" onClick={()=>{setPolyMode(false);setPolyWip([]);}}>Cancel</Btn>}
                {cfg.shape.polygon&&cfg.shape.polygon.length>2&&<Btn size="xs" variant="danger" onClick={()=>updateShape({polygon:[]})}>Clear</Btn>}
              </div>
            </div>
          )}
          ...
          <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:8}}>
            {TEMA_TUBE_ODS.map(t=>(
              <Chip key={t.val} label={t.label} active={cfg.tubeOdIn===t.val}
                onClick={()=>setCfg(p=>({...p,tubeOdIn:t.val}))} color={C.accent}/>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:4,marginBottom:8}}>
            {[1,2,4,6,8].map(n=>(
              <Chip key={n} label={`${n}P`} active={cfg.numPasses===n}
                onClick={()=>setCfg(p=>({...p,numPasses:n}))} color={C.accent}/>
            ))}
          </div>
          ...
          <button onClick={generate} disabled={busy}
            ...
            {busy?<><Spinner/> Generating…</>:"⚡ Generate Layout"}
          </button>
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {/* Toolbar */}
        <div style={{height:42,flexShrink:0,display:"flex",alignItems:"center",padding:"0 10px",
          gap:5,background:C.panel,borderBottom:`1px solid ${C.border}`,flexWrap:"nowrap",overflowX:"auto"}}>

          {layout&&(
            <div style={{display:"flex",gap:10,alignItems:"center",marginRight:4}}>
              {[{l:"Tubes",v:tubes.length,c:C.accent},{l:"Rows",v:uRows.length,c:C.success},
          ...
          <div style={{display:"flex",gap:3}}>
            <Chip label="⬚ Select" active={tool==="select"&&!polyMode} onClick={()=>{setTool("select");setPolyMode(false);}}/>
            <Chip label="✥ Pan" active={tool==="pan"&&!polyMode} onClick={()=>{setTool("pan");setPolyMode(false);}}/>
          </div>
        ...
        <div style={{flex:1,position:"relative",overflow:"hidden"}}>
          ...
          <canvas ref={canvasRef}
            style={{display:"block",width:"100%",height:"100%",
              cursor:polyMode?"crosshair":tool==="pan"||panDrag.current?"grab":"default"}}
      ...
      {/* ── RIGHT EDIT ────────────────────────────────────────────────── */}
      <div style={{width:220,flexShrink:0,background:C.panel,borderLeft:`1px solid ${C.border}`,
        display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{flex:1,overflowY:"auto",padding:"12px"}}>
          ...
          <div style={{marginBottom:6}}>
            ...
            {selIds.size>1&&(
              <div>
                ...
                <Btn variant="danger" size="xs" onClick={delSelected}>✕ Delete {selIds.size}</Btn>
              </div>
            )}
          </div>
          ...
          <div style={{maxHeight:160,overflowY:"auto",border:`1px solid ${C.border}`,borderRadius:4}}>
            {uRows.map(r=>{
              ...
              const isSel=selRows.includes(r);
              return(<div key={r} onClick={e=>e.shiftKey?addSelRow(r):selRow(r)}
          ...
          <div style={{maxHeight:130,overflowY:"auto",border:`1px solid ${C.border}`,borderRadius:4}}>
            {uCols.slice(0,60).map(c=>{
              const cnt=tubes.filter(t=>t.col===c).length;
              const isSel=selCols.includes(c);
          ...
          {layout&&(
            <>
              <Divider label="Layout Info"/>
              <div style={{background:C.bg,borderRadius:4,padding:"8px",border:`1px solid ${C.border}`}}>
                {[["OD",`${cfg.tubeOdIn}" / ${layout.tubeOdMm.toFixed(1)}mm`],
                  ["Pitch",`${layout.pitchMm.toFixed(2)}mm`],["Ratio",`${cfg.pitchRatio}×OD`],
                  ["Pattern",cfg.pattern],["Shape",cfg.shape.type],["Total",`${tubes.length} tubes`],
...
function drawShell(ctx:CanvasRenderingContext2D,shape:ShellShape,z:number,p:{x:number;y:number}){
  ctx.save();
  ctx.strokeStyle="#334155"; ctx.lineWidth=1.5; ctx.setLineDash([8,5]);
  ctx.beginPath();
  switch(shape.type){
    ...
    case"hexagon":{
      // hexSizeMm = flat-to-flat = 2×apothem. circumradius R = apothem / (√3/2)
      const apothem=(shape.hexSizeMm??300)/2;
      const R=apothem/(Math.sqrt(3)/2);
      for(let i=0;i<6;i++){
        const ang=Math.PI/3*i - Math.PI/6; // pointy-top orientation
        const x=p.x+R*z*Math.cos(ang), y=p.y+R*z*Math.sin(ang);

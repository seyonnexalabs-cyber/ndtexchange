...
export default function TankDesigner({ isTrial }: { isTrial?: boolean }) {
    ...
    const handleSave = async () => {
        if (isTrial) {
            toast.info("Feature unavailable", { description: "Please sign up to save your designs." });
            return;
        }
        if (!designId) {
                 ...
                 <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <Menubar>
                        <MenubarMenu>
                            ...
                            <MenubarContent>
                                <MenubarItem onSelect={handleSave} disabled={isTrial || !designId}>Save</MenubarItem>
                                <MenubarItem onSelect={() => !isTrial && setIsSaveModalOpen(true)} disabled={isTrial}>Save As...</MenubarItem>
                                <MenubarItem onSelect={() => !isTrial && setIsLoadModalOpen(true)} disabled={isTrial}>Load...</MenubarItem>
                            </MenubarContent>
                        </MenubarMenu>
                    </Menubar>
                </div>

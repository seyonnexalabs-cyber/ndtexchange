...
const StageCard = ({ icon: Icon, title, description, animationDelay }: { icon: React.ElementType, title: string, description: string, animationDelay: string }) => (
    <div
      className="stage-card bg-card border rounded-lg p-3 sm:p-4 w-full text-center relative z-10 h-full flex flex-col justify-center items-center"
      style={{ animationDelay }}
    >
        <div className="icon-container mx-auto bg-primary/10 text-primary p-3 rounded-full w-fit mb-2 transition-all" style={{ animationDelay }}>
    ...
    return (
        <div className="relative mt-12 w-full max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-3 gap-x-4 sm:gap-x-8 items-start relative z-10">
                ...
                <div className="h-28 sm:h-40"><StageCard icon={FilePlus2} title="1. Post Job" description="Posts a new inspection job." animationDelay={`${stepDuration * 0}s`} /></div>
                <div className="h-28 sm:h-40"><StageCard icon={Gavel} title="2. Bidding" description="Vetted providers submit competitive bids." animationDelay={`${stepDuration * 1}s`} /></div>
                <div></div>

                <div className="h-28 sm:h-40"><StageCard icon={Award} title="3. Award Job" description="Reviews bids and awards the job." animationDelay={`${stepDuration * 2}s`} /></div>
                <div className="h-28 sm:h-40"><StageCard icon={Search} title="4. Perform Inspection" description="Conducts the NDT inspection." animationDelay={`${stepDuration * 3}s`} /></div>
                <div></div>
                
                <div></div>
                <div className="h-28 sm:h-40"><StageCard icon={FileText} title="5. Submit Report" description="Uploads the digital report and findings." animationDelay={`${stepDuration * 4}s`} /></div>
                <div className="h-28 sm:h-40"><StageCard icon={ShieldCheck} title="6. Audit Report" description="(As Required) Level III review for compliance." animationDelay={`${stepDuration * 5}s`} /></div>
                
                <div className="h-28 sm:h-40"><StageCard icon={UserCheck} title="7. Client Review" description="Reviews the final report." animationDelay={`${stepDuration * 6}s`} /></div>
                <div></div>
                <div></div>

                <div className="h-28 sm:h-40"><StageCard icon={CheckCircle} title="8. Complete" description="Approves job and handles payment." animationDelay={`${stepDuration * 7}s`} /></div>
                <div></div>
                <div></div>
            </div>
        </div>
